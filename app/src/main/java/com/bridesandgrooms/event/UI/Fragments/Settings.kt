...
class Settings : Fragment(), IOnBackPressed {
    ...
    override fun onCreateView(
        ...
        if (userSession.role.isNotEmpty()) {
            val matchingRole = roles.firstOrNull { it.equals(userSession.role, ignoreCase = true) }
            if (matchingRole != null) {
                binding.roleAutocomplete.setText(matchingRole, false) // Set text without filtering
            }
        }
        ...
        if (userSession.country.isNotEmpty()) {
            val matchingCountry = countries.firstOrNull { it.equals(userSession.country, ignoreCase = true) }
            if (matchingCountry != null) {
                binding.countryAutocomplete.setText(matchingCountry, false) // Set text without filtering
            }
        }
        ...
        if (userSession.distanceunit.isNotEmpty()) {
            val matchingDistance = distances.firstOrNull { it.equals(userSession.distanceunit, ignoreCase = true) }
            if (matchingDistance != null) {
                binding.distanceAutocomplete.setText(matchingDistance, false) // Set text without filtering
            }
        }
