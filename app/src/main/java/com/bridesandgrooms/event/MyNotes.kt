...
class MyNotes : Fragment(), NotePresenter.NoteActivity, ItemSwipeListenerNote {
    ...
    override fun onNoteSuccess(inflatedView: View, noteList: MutableList<Note>) {
        if (noteList.size != 0) {
            recyclerView = inflatedView.recyclerViewNotes
            recyclerView.apply {
            ...
            val emptystateLayout = inf.findViewById<ConstraintLayout>(R.id.withnodatanotes)
            emptystateLayout.visibility = ConstraintLayout.GONE
        } else if (noteList.size == 0) {
            inflatedView.activenoteslayout.visibility = ConstraintLayout.GONE
